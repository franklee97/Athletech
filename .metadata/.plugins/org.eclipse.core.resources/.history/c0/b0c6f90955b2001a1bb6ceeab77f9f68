...
ring_buffer tx_debug_buffer = { { 0 }, 0, 0 };

ring_buffer *_rx_buffer;
ring_buffer *_tx_buffer;

ring_buffer *_tx_debug_buffer;
...
void Ringbuf_init(void) {
    ...
    _tx_debug_buffer = &tx_debug_buffer;
    ...
    /* Enable the UART Error Interrupt: (Frame error, noise error, overrun error) */
    __HAL_UART_ENABLE_IT(debug_uart, UART_IT_ERR);
...
void Uart_debug_write(int c) {
    if (c >= 0) {
        int i = (_tx_debug_buffer->head + 1) % UART_BUFFER_SIZE;

        // If the output buffer is full, there's nothing for it other than to
        // wait for the interrupt handler to empty it a bit
        // ???: return 0 here instead?
        while (i == _tx_debug_buffer->tail)
            ;

        _tx_debug_buffer->buffer[_tx_debug_buffer->head] = (uint8_t) c;
        _tx_debug_buffer->head = i;

        __HAL_UART_ENABLE_IT(debug_uart, UART_IT_TXE); // Enable UART transmission interrupt
    }
}
...
void Uart_debug_isr(UART_HandleTypeDef *huart) {
    ...
    if (((isrflags & USART_ISR_TXE) != RESET)
            && ((cr1its & USART_CR1_TXEIE) != RESET)) {
        if (tx_debug_buffer.head == tx_debug_buffer.tail) {
            // Buffer empty, so disable interrupts
            __HAL_UART_DISABLE_IT(huart, UART_IT_TXE);
            ...
            // There is more data in the output buffer. Send the next byte
            unsigned char c = tx_debug_buffer.buffer[tx_debug_buffer.tail];
            tx_debug_buffer.tail =
                    (tx_debug_buffer.tail + 1) % UART_BUFFER_SIZE;
